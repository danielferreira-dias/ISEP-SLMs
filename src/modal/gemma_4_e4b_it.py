"""Run Gemma 4 E4B through the benchmark pipeline on Modal."""
...
APP_NAME = "isep-gemma-4-e4b-it-benchmark"
MODEL_CONFIG_ID = "gemma_4_e4b_it"
...
class GemmaE4bVllmServer:
    """OpenAI-compatible Gemma 4 E4B server on one L40S."""

    @modal.enter()
    def start(self) -> None:
        """Start vLLM with Gemma thinking disabled."""
...
def main(
    ...
    if not dry_run:
        server_url = GemmaE4bVllmServer.get_url()
        if not server_url:
