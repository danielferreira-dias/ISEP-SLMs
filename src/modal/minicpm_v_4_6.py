"""Run MiniCPM-V 4.6 through the benchmark pipeline on Modal."""
...
APP_NAME = "isep-minicpm-v-4-6-benchmark"
MODEL_CONFIG_ID = "minicpm_v_4_6"
...
runtime_image = (
    ...
    .run_commands(
        "sed -i "
        "'s/self.version = image_processor.version/"
        "self.version = getattr(image_processor, \"version\", None)/' "
        "/usr/local/lib/python3.12/site-packages/vllm/"
        "transformers_utils/processors/minicpmv.py",
        "grep -q 'getattr(image_processor, \"version\", None)' "
        "/usr/local/lib/python3.12/site-packages/vllm/"
        "transformers_utils/processors/minicpmv.py",
    )
    .env(
...
class MiniCpmVllmServer:
    """OpenAI-compatible MiniCPM-V server on one L40S."""

    @modal.enter()
    def start(self) -> None:
        """Start vLLM with the MiniCPM image-processor parameters."""
...
def main(
    ...
    if not dry_run:
        server_url = MiniCpmVllmServer.get_url()
        if not server_url:
