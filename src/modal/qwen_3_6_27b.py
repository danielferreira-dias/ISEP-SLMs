"""Run the repository benchmark pipeline against Qwen 3.6 27B on Modal.

The remote Modal Server exposes vLLM's OpenAI-compatible API. The local
entrypoint waits for that server and then invokes the same benchmark CLI used
for API models, so selection, prompts, validation, metrics, and result
artifacts remain identical across providers.
"""
...
class QwenVllmServer:
    """OpenAI-compatible vLLM server hosted on one Modal GPU."""

    @modal.enter()
    def start(self) -> None:
        """Start vLLM with the runtime settings from the model YAML."""
